    ...
    pub async fn load_folder_from_sharepoint_impl(
        ...
        for file_info in files {
            ...
            match file_info.name.split('.').last().unwrap_or("").to_lowercase().as_str() {
                "csv" => {
                    match load_csv_from_sharepoint(tenant_id, client_id, site_url, &file_path).await {
                        Ok(df) => {
                ...
                "xlsx" | "xls" => {
                    match load_excel_from_sharepoint(tenant_id, client_id, site_url, &file_path).await {
                        Ok(df) => {
                ...
                "json" => {
                    match load_json_from_sharepoint(tenant_id, client_id, site_url, &file_path).await {
                        Ok(df) => {
                ...
                "parquet" => {
                    match load_parquet_from_sharepoint(tenant_id, client_id, site_url, &file_path).await {
                        Ok(df) => {
